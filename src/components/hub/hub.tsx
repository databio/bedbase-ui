...
function SearchInput({ onFileSelect }: { onFileSelect: (f: File) => void }) {
  ...
  return (
    <div className="w-full max-w-2xl">
      {/* Text search row */}
      <div className={`flex items-center gap-2 border-[1.5px] border-primary/30 px-3 py-2.5 ${searchMode === 'bed' ? 'rounded-t-lg' : 'rounded-lg'}`}>
        <input
          ref={searchInputRef}
          type="text"
      ...
      {/* File upload row or file indicator (hidden in BEDset mode) */}
      {searchMode === 'bed' && (
        bedFile ? (
          <div
